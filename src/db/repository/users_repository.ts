import { Token, TokenModel, Usuario, UsuarioModel } from "../models/usuarioModel"
...
export const crearUserRepository = async (payload: UsuarioModel): Promise<UsuarioModel> => {
    const user = await Usuario.create(payload)
    const response = user.toJSON() as UsuarioModel
    return response
}

export const consultarUsuarioById = async (id: number): Promise<UsuarioModel> => {
...
export const consultarUsuarioByUsername = async (username: string): Promise<UsuarioModel> => {
...
export const registrarUsuario = async (userInput: UsuarioModel): Promise<UsuarioModel> => {
