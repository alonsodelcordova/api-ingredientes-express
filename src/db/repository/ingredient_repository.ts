...
export const create = async (payload: IngredientModel): Promise<IngredientModel> => {
    const ingredient = await Ingredient.create(payload)
    const response = ingredient.toJSON() as IngredientModel
    return response
}

export const update = async (id: number, payload: Partial<IngredientModel>): Promise<IngredientModel> => {
